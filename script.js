...
function showQuestion(question) {
    questionBox.innerHTML = ''; // 이전 질문의 답변 버튼을 제거합니다.
...
function showResult() {
    ...
    // 복합형 문항 점수 계산
    const combinedLpScore = totalLeadScore + totalExpressionScore;
    const combinedLcScore = totalFlowScore + totalResponseScore;
    const combinedThreshold = 45; // 복합형으로 분류될 점수 기준

    // 복합형으로 분류될 점수가 높을 경우
    if (combinedLpScore > combinedThreshold && combinedLcScore > combinedThreshold) {
        if (combinedLpScore > combinedLcScore) {
            resultType = 'LP';
            resultDescriptionHTML = `
                ...
                순간적인 퍼포먼스와 에너지 전달에 탁월하며, 팀의 활력을 끌어올려 단기적인 성과를 내는 데 가장 큰 강점이 있습니다.</p>
                <h3>강점</h3>
                <ul>
            ...
            resultDescriptionHTML = `
                ...
                순간적인 주도권이나 단기적인 몰입보다, 장기적인 관점에서 팀의 지속적인 성장과 안정성을 확보하는 데 가장 큰 강점이 있습니다.</p>
                <h3>강점</h3>
                <ul>
