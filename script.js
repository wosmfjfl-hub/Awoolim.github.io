...
const shareKakao = document.getElementById('share-kakao'); // New: Share buttons
...
let answerHistory = []; // New: Stores the user's answers to go back
...
                <h2>현장을 압도하고 몰입과 퍼포먼스로 팀을 이끄는 리더</h2>
...
        descriptionHTML: `
            <div class="result-description-section">
                <h2>유연하게 흐름을 조율하며 팀에 안정과 신뢰를 주는 리더</h2>
...
        descriptionHTML: `
            <div class="result-description-section">
                <h2>순간의 열정으로 팀을 밝히고 몰입시키는 점화자</h2>
...
        descriptionHTML: `
            <div class="result-description-section">
                <h2>팀의 기반을 단단히 다지며 안정감을 선사하는 리더</h2>
...
        descriptionHTML: `
            <div class="result-description-section">
                <h2>팀 분위기를 활기차게 만들고 몰입을 이끄는 생동감 전도사</h2>
...
        descriptionHTML: `
            <div class="result-description-section">
                <h2>팀의 감정을 읽고 균형과 조화를 만드는 조율자</h2>
...
    questionTitle.innerText = `${questionNumber}. ${question.question.substring(question.question.indexOf('.') + 1).trim()}`;
...
    });
    
    // Check if the user has a selected answer for this question in history
...
        leadFlowDifference <= (totalMaxLeadFlowScore * 0.05) &&
        (leadScore + flowScore) >= (totalMaxLeadFlowScore * 0.7);
...
    // New: Save result to statistics
    saveResultToStats(resultType);

    const result = resultsData[resultType];
    resultContainer.className = `result-container type-${resultType}`; 
...
// New: Function to save quiz results to local storage for statistics
function saveResultToStats(resultType) {
    try {
        const stats = JSON.parse(localStorage.getItem('attStats')) || {};
        stats[resultType] = (stats[resultType] || 0) + 1;
        localStorage.setItem('attStats', JSON.stringify(stats));
    } catch (e) {
        console.error('로컬 스토리지 저장 실패:', e);
    }
}

// New: Function to display statistics (You'll need a new HTML section for this)
...
        if (totalCount === 0) {
            console.log('아직 저장된 통계 데이터가 없습니다.');
            return;
        }

        console.log(`총 ${totalCount}명이 참여했습니다.`);
        console.log('--- 유형별 통계 ---');
        for (const type in stats) {
            const percentage = ((stats[type] / totalCount) * 100).toFixed(1);
            console.log(`${resultsData[type].title}: ${stats[type]}명 (${percentage}%)`);
        }
    } catch (e) {
...
    if (currentQuestionIndex > 0) {
        // Get the previous question's answer from history
...
        // Save the current answer to history
...
        const currentSelectedBtn = document.querySelector('.answer-btn.selected');
...
    const resultDescriptionSections = document.querySelectorAll('.result-description-section');
    
    let resultText = `Awoolim Type Test (ATT) 결과: ${resultTitleText}\n\n`;
    
    resultDescriptionSections.forEach(section => {
        const sectionTitle = section.querySelector('h2, h3');
        if (sectionTitle) {
            resultText += `=== ${sectionTitle.innerText} ===\n`;
        }
        
        const paragraphs = section.querySelectorAll('p');
        paragraphs.forEach(p => {
            resultText += p.innerText + '\n\n';
        });
        
        const listItems = section.querySelectorAll('li');
        listItems.forEach(li => {
            resultText += `- ${li.innerText}\n`;
        });
        resultText += '\n';
    });
...
// New: Social share button event listeners
shareKakao.addEventListener('click', () => {
    const resultTitleText = resultTitle.innerText;
    const currentUrl = window.location.href;

    // `Kakao` 객체가 존재하는지 확인합니다.
    if (typeof Kakao !== 'undefined' && Kakao.isInitialized()) {
        Kakao.Share.sendDefault({
            ...
            content: {
                ...
                imageUrl:
                    'https://developers.kakao.com/assets/img/about/logos/kakaotalksharing/kakaotalk_sharing_btn_medium.png', // ✅ 이 이미지 URL을 실제 결과 이미지로 변경해 보세요.
                link: {
        ...
        alert("카카오톡 SDK가 제대로 초기화되지 않았습니다. 개발자 키와 플랫폼 등록을 확인해주세요.");
    }
});
